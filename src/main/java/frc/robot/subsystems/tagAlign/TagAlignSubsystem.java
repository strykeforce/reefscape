...
public class TagAlignSubsystem extends SubsystemBase {
    ...
    public int computeHexant(/* Alliance color */) {
        Translation2d reefT = /* color == Alliance.Blue ? TagServoingConstants.kBlueReefPose : */ TagServoingConstants.kRedReefPose;
        double offset = Units.degreesToRadians(30);

        return (((int) (FastMath.normalizeZeroTwoPi(
                driveSubsystem.getPoseMeters().getTranslation().minus(reefT).getAngle().getRadians() - offset)
                / Units.degreesToRadians(60) + offset)) + 3) % 6;
    }

    // Red reef numbered like blue (red 0 is facing the same direction as blue 0)
    public int computeFieldRelHexant(/* Alliance color */) {
        return (computeHexant(/* color */) + /* color == Alliance.Blue ? 0 : */ 3) % 6;
    }

    public Pose2d getTargetDrivePose(/* Alliance color */) {
        Translation2d reefT = /* color == Alliance.Blue ? TagServoingConstants.kBlueReefPose : */ TagServoingConstants.kRedReefPose;

        Translation2d offset = new Translation2d(TagServoingConstants.kInitialDriveRadius,
    ...
    public void setup(/* Alliance color, */ boolean scoreLeft) {
        ...
        targetCamId = !scoreLeft ? TagServoingConstants.kLeftServoCam : TagServoingConstants.kRightServoCam;
        targetTagId = /*
    ...
    public void periodic() {
        switch (curState) {
            case DRIVE -> {
                ...
                double vOmega = driveX.calculate(current.getRotation().getRadians(),
                ...
                if (Math.abs(driveOmega.getPositionError()) < TagServoingConstants.kAngleCloseEnough
                        && targetPose.minus(current).getTranslation()
                                .getNorm() < TagServoingConstants.kDriveCloseEnough) {
                    alignX.reset(current.getX());
                    alignY.reset(current.getY());
                    alignOmega.reset(driveSubsystem.getGyroRotation2d().getRadians());
            ...
            case TAG_ALIGN -> {
                ...
                if (result.getNumTags() > 0) {
                    ...
                    if (TagServoingConstants.kAreaTarget - area < TagServoingConstants.kAngleCloseEnough
                        ...
                        if (Math.abs(TagServoingConstants.kHorizontalTarget
                                - center.x()) < TagServoingConstants.kHorizontalCloseEnough) {
                    ...
                    double vOmega = alignOmega.calculate(driveSubsystem.getGyroRotation2d().getRadians(),
                            targetPose.getRotation().getRadians());
